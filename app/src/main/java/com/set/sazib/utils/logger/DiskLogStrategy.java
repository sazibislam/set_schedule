...
public class DiskLogStrategy implements LogStrategy {
  ...
  static class WriteHandler extends Handler {
    ...
    private final String LOG_ = "logs";

    WriteHandler(@NonNull Looper looper, @NonNull String folder, int maxFileSize) {
      super(LogUtils.checkNotNull(looper));
      this.folder = LogUtils.checkNotNull(folder);
    ...
    public void handleMessage(
      ...
      File logFile = getLogFile(folder, LOG_);
    ...
    private File getLogFile(@NonNull String folderName, @NonNull String fileName) {
      ...
      LogUtils.checkNotNull(fileName);

      File folder = new File(folderName);
      boolean wasSuccessful = folder.mkdirs();
      if (!folder.exists()) {
        if (!wasSuccessful) {
          AppLogger.e("getLogFile: was not successful.");
        }
      }
